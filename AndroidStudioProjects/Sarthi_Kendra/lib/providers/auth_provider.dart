...
class AuthProvider extends ChangeNotifier {
  ...
  // Unread notification count — drives the bell dot without opening the screen
  int          _unreadCount = 0;
  ...
  // Runs silently — does NOT wait for any screen to open
  void _subscribeNotifications(String uid) {
    ...
    _notifSub = _db
      ...
      notifyListeners();          // Updates bell dot everywhere instantly
  ...
  Future<bool> register({
    ...
    try {
      // ── Check if email already exists in Firestore helpers ────
      // (handles the case where Auth has the account but Firestore doc was deleted)
      ...
      // If email is in Auth but NOT in Firestore — orphan account
